...
import GuidedScenariosTab from "@/components/guided-scenarios-tab"
...
export default function BankersAlgorithm() {
  ...
  const [simSubTab, setSimSubTab] = useState("setup") // Declare simSubTab and setSimSubTab
    ...
    <div className="grid grid-cols-1 gap-4 w-full max-w-full overflow-x-hidden">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="w-full bg-white p-0 h-auto flex flex-wrap justify-center gap-0">
          <TabsTrigger
            value="simulation"
            className="flex-1 max-w-xs data-[state=active]:bg-white data-[state=active]:border data-[state=active]:border-gray-300 data-[state=inactive]:bg-gray-100 text-sm py-3 rounded-none border-b-0"
          >
            Simulation
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="simulation" className="mt-0">
          <Tabs value={simSubTab} onValueChange={setSimSubTab} className="w-full">
            <TabsList className="w-full bg-white p-0 h-auto flex flex-wrap justify-center gap-0 mt-3">
              <TabsTrigger
                value="setup"
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          className="bg-[#3498db] hover:bg-[#2980b9] text-xs sm:text-sm"
                        >
                          Check System State
                        </Button>
                      </TooltipTrigger>
                          ...
                          <tr key={`${process.id}-${rIndex}`} className={pIndex % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                            ...
                            <td className="border p-2 text-xs sm:text-sm">
                              <Input
                                ...
                                onChange={(e) =>
                                  handleAllocationChange(pIndex, rIndex, Number.parseInt(e.target.value))
                                }
                                className="w-16 sm:w-20"
                              />
                            </td>
                          ...
                          {Array.from({ length: resourceTypes }).map((_, rIndex) => (
                            <div key={`${process.id}-req-${rIndex}`}>
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  handleRequestChange(process.id, rIndex, Number.parseInt(e.target.value))
                                }
                ...
                <div>
                  ...
                  <div className="border rounded-md p-3 sm:p-4">
                    ...
                    {safeSequence.length > 0 ? (
                      <div>
                        <div
                          className={`text-xs sm:text-sm font-medium ${isDeadlock ? "text-red-500" : "text-green-500"}`}
                        >
                      ...
                      <div className="text-xs sm:text-sm text-gray-500">
                        Run the safety algorithm to check system state
                      </div>
                    )}
                  </div>
                </div>
        ...
        <TabsContent value="manual-guide" className="mt-0">
          <TutorialTab />
        </TabsContent>

        <TabsContent value="guided-scenarios" className="mt-0">
          <GuidedScenariosTab />
        </TabsContent>

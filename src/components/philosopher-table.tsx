	...
	return (
		<div className="relative w-full aspect-square max-w-[300px] mx-auto">
			...
			{chopsticks.map((chopstick, index) => {
				const philosopherPos = getPosition(
					index,
					philosophers.length,
				...
				const angle = Math.atan2(
					nextPhilosopherPos.y - philosopherPos.y,
					nextPhilosopherPos.x - philosopherPos.x
				);
					...
					<TooltipProvider key={`chopstick-${chopstick.id}`}>
						<Tooltip>
							<TooltipTrigger asChild>
								<button
									className={`absolute rounded-full ${getChopstickColor(
										chopstick.state
									...
									onClick={() =>
										!isSimulating && onChopstickClick(chopstick.id)
									}
									onMouseEnter={() =>
										setHoveredElement({ type: "chopstick", id: chopstick.id })
									}
									onMouseLeave={() => setHoveredElement(null)}
									disabled={isSimulating}
									aria-label={`Chopstick ${chopstick.id + 1}, currently ${
										chopstick.state
									}`}
								/>
							</TooltipTrigger>
							<TooltipContent>
								<div className="text-sm">
									...
									{chopstick.heldBy !== null && (
										<p>Held by: Philosopher {chopstick.heldBy + 1}</p>
									)}
									{!isSimulating && (
										<p className="font-semibold mt-1">
											Click to{" "}
											{chopstick.state === "available" ? "pick up" : "put down"}
										</p>
									)}
								</div>
							</TooltipContent>
				...
				return (
					<TooltipProvider key={`philosopher-${philosopher.id}`}>
						<Tooltip>
							<TooltipTrigger asChild>
								<button
									className={`absolute rounded-full border-2 ${getPhilosopherColor(
										philosopher.state
									)} ${getPhilosopherBorderColor(
										philosopher.state
									...
									onClick={() =>
										!isSimulating && onPhilosopherClick(philosopher.id)
									}
									onMouseEnter={() =>
										setHoveredElement({
											type: "philosopher",
									...
									onMouseLeave={() => setHoveredElement(null)}
									disabled={isSimulating}
									aria-label={`Philosopher ${philosopher.id + 1}, currently ${
										philosopher.state
									}`}
								>
							...
							<TooltipContent>
								<div className="text-sm">
									...
									{philosopher.leftChopstick !== null && (
										<p>Has left chopstick: {philosopher.leftChopstick + 1}</p>
									)}
									{philosopher.rightChopstick !== null && (
										<p>Has right chopstick: {philosopher.rightChopstick + 1}</p>
									)}
									{!isSimulating && (
										<p className="font-semibold mt-1">
											{philosopher.state === "thinking" &&
												"Click to make hungry"}
											{philosopher.state === "hungry" && "Click to try eating"}
											{philosopher.state === "eating" &&
												"Click to finish eating"}
										</p>
									)}
								</div>
							</TooltipContent>
			...
			{/* {philosophers.map((philosopher, index) => {
				const position = getPosition(
					index,
					philosophers.length,
					tableRadius * 1.8 + philosopherRadius * 0
				);

				return (
					<div
						key={`label-${philosopher.id}`}
						className="absolute text-xs font-semibold text-left"
						style={{
							left: position.x,
							top: position.y,

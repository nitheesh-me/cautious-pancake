          ...
          <TooltipProvider key={`chopstick-${chopstick.id}`}>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  className={`absolute transform -translate-x-1/2 -translate-y-1/2 rounded-full ${getChopstickColor(chopstick.state)} transition-colors duration-300 ${isSimulating ? "cursor-not-allowed" : "cursor-pointer hover:opacity-80"}`}
                  style={{
